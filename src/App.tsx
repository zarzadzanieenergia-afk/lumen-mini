...
export default function App() {
  ...
  const logoSrc = `${import.meta.env.BASE_URL}lumen-icon.png`;
  ...
  const save = (section: LightingSection) => {
    setSections((current) =>
      current.some((x) => x.id === section.id)
        ? current.map((x) => (x.id === section.id ? section : x))
        : [...current, section]
    );
  ...
  const titles: Record<ViewId, [string, string]> = {
    ...
    dodaj: [
      editing ? "Edycja odcinka" : "Dodaj odcinek",
      "Wprowadź dane techniczne, kosztowe i lokalizację",
    ],
    ...
    <div className="min-h-screen lg:grid lg:grid-cols-[250px_1fr]">
      <aside className="hidden border-r border-slate-200 bg-slate-100/70 p-5 lg:sticky lg:top-0 lg:flex lg:h-screen lg:flex-col">
        <div className="mb-8 flex items-center gap-3 px-2">
          ...
          />
          <div>
            <p className="text-lg font-black tracking-tight text-ink">LUMEN Mini</p>
        ...
        <div className="mt-auto rounded-2xl bg-ink p-4 text-white">
          ...
          <p className="mt-2 text-sm leading-relaxed text-white/85">
            Dane pozostają w tej przeglądarce. Aplikacja działa również jako PWA.
          </p>
        </div>
      </aside>
        ...
        <header className="sticky top-0 z-[1000] flex items-center justify-between gap-4 border-b border-slate-200 bg-white/95 px-4 py-3 backdrop-blur sm:px-7 lg:px-10 lg:py-5">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="hidden text-xs text-slate-500 sm:block">{titles[view][1]}</p>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            <button
              ...
              onClick={() => {
                ...
                notify("Wczytano dane przykładowe.");
              }}
            >
        ...
        <main className="p-4 pb-28 sm:p-7 lg:p-10">
          ...
          {view === "lista" && (
            <SectionsList
              ...
              onDelete={(id) => {
                setSections((current) => current.filter((x) => x.id !== id));
                notify("Usunięto odcinek.");
              }}
            />
          )}
      ...
      {toast && (
        <div
          className={`fixed bottom-24 right-4 z-[2000] max-w-sm rounded-xl px-5 py-3 text-sm font-semibold text-white shadow-2xl lg:bottom-6 ${
            toast.error ? "bg-red-700" : "bg-teal"
          }`}
        >
          {toast.message}
        </div>
